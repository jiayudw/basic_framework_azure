/**
 * @file chassis.c
 * @author NeoZeng [neozng1@hnu.edu.cn](mailto:neozng1@hnu.edu.cn)
 ...
 * @note  采用等比例缩放速度的策略，避免麦轮底盘运动学解算发生畸变。
 *        引入了常量定义与离线保护机制，保证平时调试的可用性。
 */
...
#ifdef CHASSIS_BOARD
    // 如果是双板架构，裁判系统数据需要通过 CAN 从云台板传过来
    // 这里确保你给 feedback 结构体填充了真实的 buffer_energy
...
//     // 【猜想逻辑】如果 power_limit 为 0，很可能是没有插裁判系统或串口断连
//     // 此时应当放开限制（维持 1.0），否则平时调试底盘会非常卡顿
//     if (power_limit > 0.0f) 
//     {
//         if (current_buffer < BUF_EXTREME_THRES) 
//         {
