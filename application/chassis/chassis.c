...
void ChassisTask()
{
    ...
    case CHASSIS_ROTATE: // 自旋,同时保持全向机动;当前wz维持定值,后续增加规则的变速策略
        ...
        // }
        // else if(chassis_cmd_recv.wz == 300 ){
        //     chassis_cmd_recv.wz -= 10; 
        // }
        break;
    default:
        break;
    ...
    // chassis_feedback_data.last_time = (uint16_t)referee_data->GameState.stage_remain_time;
    if (referee_data->GameState.game_progress == 4){
        chassis_feedback_data.gamestate = 1;
    }
    else{
        chassis_feedback_data.gamestate = 0;
    }
    // 添加底盘速度控制指令反馈代码
